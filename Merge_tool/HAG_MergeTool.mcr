/*
    These MacroScripts are designet to look for the source script in either Scripts / User scripts folders.
*/
-- Node Merge tool
	...
	(
		...
		(
			_Tool = if _Tool == undefined then (
				try (
					fileIn("$Scripts\\HAG_ExplodeMerge.ms")
				) catch (
					fileIn("$UserScripts\\HAG_ExplodeMerge.ms")
				)
			) else _Tool
